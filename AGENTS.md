...
1. **MCP Server** — An MCP server with 22 structured query tools backed by SQLite. For AI agents that use tool calls (e.g., GitHub Copilot with MCP support).
...
tests/
  ...
  RepoMind.Scanner.Tests/                   # Scanner engine tests (18 tests)
...
| `detect_patterns` | Auto-detect architecture patterns (Repository, Decorator, CQRS, Factory, etc.) |
